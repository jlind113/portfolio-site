...
interface Props {
  ...
  /**
   * Optional key used to cache waveform bars in localStorage
   * Example: audio file path or stable track id
   */
  cacheKey?:string;
}
    ...
    (
        ...
        useEffect(() => {
            const processBlob = async (): Promise<void> => {
                ...
                if (cacheKey) {
                    const cached = localStorage.getItem("waveform:" + cacheKey);
                    if (cached) {
                        try {
                ...
                if (cacheKey) {
                    localStorage.setItem("waveform:" + cacheKey, JSON.stringify(barsData));
                }
            ...
            processBlob();
        }, [blob, cacheKey, width, height, barWidth, gap, backgroundColor, barColor, barPlayedColor]);
        ...
        }, [data, currentTime, duration, barWidth, gap, backgroundColor, barColor, barPlayedColor]);
